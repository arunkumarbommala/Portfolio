...
const sets = {
  ...
  fullstack: [
    ...
    ["devicon-dotnetcore-plain", ".NET Core"],
    ["devicon-dot-net-plain", "ASP.NET"],
  ...
  frontend: [
    ...
    ["devicon-bootstrap-plain", "Bootstrap"],
    ["devicon-jquery-plain", "jQuery"],
    ["devicon-ajax-plain", "AJAX"]
  ],

  backend: [
    ["devicon-csharp-plain", "C#"],
    ["devicon-dotnetcore-plain", ".NET Core"],
    ["devicon-dot-net-plain", "ASP.NET MVC"],
    ["devicon-microsoftsqlserver-plain", "SQL Server"],
  ...
  "GitHub": 80
    ...
    // 6. Connect Service Cards to Skills Popup
    document.querySelectorAll(".services-list div[data-service]").forEach(card => {
      card.addEventListener("click", (e) => {
        // Prevent popup logic if the user clicked the "Learn more" link
        if (e.target.closest("a")) return;

        const service = card.getAttribute("data-service");
        // Open popup with percentage bars enabled (true)
        if (service) openPopup(service, true);
      });
    });
...
// Domains Card Scrolling Code

const track = document.getElementById("domainTrack");
